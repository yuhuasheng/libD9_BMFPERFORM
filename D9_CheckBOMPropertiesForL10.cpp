...
int checkBOMAttrForL10(char* parentId, char* parentVersion, ResultSet* pRs, char** error_msg);
		...
		for (size_t i = 0; i < attach_object_num; i++)
		{
			...
			else
			{
				...
				*findTableSql = "BEGIN bomline_procedure(:1, :2, :3); end;";
				...
				ITKCALL(checkBOMAttrForL10(parentId, parentVersion, pRs, &error_msg)); // 判断BOMLine属性是否填写
				//while (pRs->next())
				//{
				//	char tempStr[256];
...
int checkBOMAttrForL10(char* parentId, char* parentVersion, ResultSet* pRs, char** error_msg)
{
	...
	{
		char tempStr[1024];
		puid = pRs->getString(9);
		printf("puid == %s\n", puid.c_str());
		ITK__convert_uid_to_tag(puid.c_str(),&rev_tag);
